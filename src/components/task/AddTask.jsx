...
const AddTask = ({ onClose }) => {
    ...
    const formatDate = (date) => {
        ...
        const month = (`0${d.getMonth() + 1}`).slice(-2); // Add leading zero for months < 10
        const day = (`0${d.getDate()}`).slice(-2); // Add leading zero for days < 10
        return `${year}-${month}-${day}`;
    };

    const handleDateChange = (e) => {
        setDate(e.target.value); // Set formatted date when user changes it
    };
    ...
    useEffect(() => {
        // Set the initial date value in the correct format
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-gray-50 rounded-lg p-4 max-w-screen-md w-full mx-6">
                <div id='header-1'>
                    <h2 className="text-lg border-b-2 w-fit border-teal-500 text-start text-teal-700 font-bold mb-4">Add Task</h2>
